...
<div class="container">
    <div class="row">
        ...
        <div class="col-12">
            <h1 class="display-1 ">Fixar Suspended Services</h1>

            <!-- Laravel Success Handling -->
            @if(session()->has('success'))
                <p class="lead fst-italic text-success">{{session()->get('success')}}</p>
            @else
                <p class="lead fst-italic">Best Services Should Be Up Front !</p>
            @endif
        </div>
...
<script>
    $(document).ready(function () {
        let options = {
            "ajax": "{{url()->to('admin/services/suspended/data')}}",
            "processing": true,
            "serverSide": true,
            "serverMethod": 'get',
            "columns": [
        ...
        $('.dataTable').on('click',".recover",function () {
            ...
            $.ajax({
                url: "{{URL::to('/admin/services/recover')}}",
                type: "POST",
